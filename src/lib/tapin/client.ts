...
// حداکثر تعداد آیتمی که در هر صفحه درخواست می‌شود. قبلاً ۵۰۰ بود — یک
// مقدار غیرمعمول و خیلی بزرگ‌تر از نمونه‌ی مستندات رسمی تاپین (که count:4
// را نشان می‌داد)؛ چون احتمال می‌رفت یک count به این بزرگی رفتار
// ناپایدار/غیرمنتظره از سمت سرور تاپین ایجاد کند، به ۵۰ کاهش یافت — عددی
// در بازه‌ی رایج APIهای صفحه‌بندی‌شده و به‌اندازه‌ی کافی بزرگ که هر ۳۱
// استان کشور را در یک صفحه بگیرد. طبق تایید مستندات رسمی تاپین، سرور
// صرف‌نظر از این مقدار ممکن است per-page کوچک‌تری برگرداند و `total_count`
// واقعی را در پاسخ اعلام کند — به همین دلیل کد پایین‌تر
// (fetchTapinPaginatedList) به کافی‌بودن یک صفحه اعتماد نمی‌کند و صفحات
// بعدی را تا رسیدن به `total_count` می‌گیرد.
...
// نشود. ۲۰ صفحه × ۵۰ آیتم = تا ۱٬۰۰۰ آیتم، خیلی بیشتر از تعداد واقعی
