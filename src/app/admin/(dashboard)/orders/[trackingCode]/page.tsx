  ...
  const order = await prisma.order.findUnique({
    ...
    include: {
      ...
      statusHistory: {
        ...
        include: { changedByEmployee: true, changedByCompanyAccount: true },
      },
    },
  });
    ...
    <div className="flex flex-col gap-6">
      <div className="rounded-2xl border border-neutral-200 bg-white p-5 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="text-neutral-700 font-medium">
          {formatToman(Number(order.calculatedPrice))} (کمیسیون: {formatToman(Number(order.commissionAmount))})
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-neutral-200 bg-white p-5">
        ...
        <AdminOrderStatusChanger
          ...
          canEdit={current.can("orders", "edit")}
        />
      </div>
          ...
          {order.statusHistory.map((h) => (
            <li key={h.id} className="flex items-center gap-2 text-sm text-neutral-600">
              ...
              <span className="text-xs text-neutral-400">
                توسط:{" "}
                {h.changedBy === "admin" || h.changedBy === "employee"
                  ? h.changedByEmployee?.name ?? "ادمین"
                  : h.changedBy === "company_account"
                    ? h.changedByCompanyAccount?.username ?? "شرکت"
                    : h.changedBy === "system"
                      ? "سیستم"
                      : "شرکت (Webhook)"}
              </span>
